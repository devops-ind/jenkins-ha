...
```bash
# Add to /etc/hosts
echo "192.168.86.30 devops.jenkins.example.com" | sudo tee -a /etc/hosts
echo "192.168.86.30 prometheus.jenkins.example.com" | sudo tee -a /etc/hosts
```
...
curl http://192.168.86.30:9090/graph  # Prometheus
```
...
curl http://192.168.86.30:8080/login   # Blue
curl http://192.168.86.30:8180/login   # Green
...
curl http://192.168.86.30:9090/graph      # Prometheus
curl http://192.168.86.30:9300/login      # Grafana  
curl http://192.168.86.30:9100/metrics    # Node Exporter
```
...
curl -u admin:admin123 http://192.168.86.30:8404/stats | grep -E 'prometheus|grafana|node'
```
...
docker exec jenkins-haproxy curl -f http://192.168.86.30:8080/login
docker exec jenkins-haproxy curl -f http://192.168.86.30:9090/api/v1/status/config
...
docker exec jenkins-haproxy ping -c 3 192.168.86.30
docker exec jenkins-devops-blue ping -c 3 192.168.86.30
...
watch -n 2 'curl -s -u admin:admin123 http://192.168.86.30:8404/stats | grep -E "devops|developer|jenkins|prometheus|grafana"'
...
for service in "${SERVICES[@]}"; do
  ...
  if curl -s -f -H "Host: $domain" "http://192.168.86.30:$port$path" > /dev/null; then
    echo "✅ OK"
  else
    echo "❌ FAILED"
  fi
...
for port_info in "${ports[@]}"; do
  ...
  if nc -z 192.168.86.30 "$port" 2>/dev/null; then
    echo "✅ Open"
  else
    echo "❌ Closed"
  fi
...
# Jenkins: http://192.168.86.30:8080
