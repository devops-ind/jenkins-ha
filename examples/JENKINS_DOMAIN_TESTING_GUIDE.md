...
for team in devops developer jenkins; do
  ...
  curl -f -H "Host: $team.jenkins.example.com" \
    http://192.168.1.10:8000/api/json?pretty=true
done
```
...
curl http://192.168.1.10:8082/login  # Jenkins blue
```
...
curl -k -H "Host: devops.jenkins.example.com" https://192.168.1.10:8000/login
```
...
curl -I -H "Host: devops.jenkins.example.com" http://192.168.1.10:8000/login | grep -E "X-Content-Type|X-Frame|X-XSS"
```
